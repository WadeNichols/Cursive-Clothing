import React from 'react'

//product picture imports
import cejon from './assets/womens/cejon.jpg';
import democracy from './assets/womens/democracy.jpg';
import furHood from './assets/womens/furHood.jpg';
import generationJacket from './assets/womens/generationJacket.jpg';
import gloves from './assets/womens/gloves.jpg';
import guessJacket from './assets/womens/guessJacket.jpg';
import halogen from './assets/womens/halogen.jpg';
import korsPurse from './assets/womens/korsPurse.jpg';
import kyi from './assets/womens/kyi.jpg';
import laceTrim from './assets/womens/laceTrim.jpg';
import luckyBrand from './assets/womens/luckyBrand.jpg';
import lush from './assets/womens/lush.jpg';
import plaidCoat from './assets/womens/plaidCoat.jpg';
import rayban from './assets/womens/rayban.jpg';
import raybanShield from './assets/womens/raybanShield.jpg';
import rebeccaPurse from './assets/womens/rebeccaPurse.jpg';
import satinBlouse from './assets/womens/cejon.jpg';
import toms from './assets/womens/toms.jpg';
import uggs from './assets/womens/uggs.jpg';
import uggsGrey from './assets/womens/uggsGrey.jpg';
import unionScarf from './assets/womens/unionScarf.jpg';
import vertigo from './assets/womens/vertigo.jpg';


const WomensClothing = () => {
  return (
    <div className="items-container">
      {/* coats */}
      <div className="coats">
        <a
          href="https://www.nordstromrack.com/shop/product/2648627/french-connection-waist-belt-quilted-faux-fur-hooded-jacket?color=BLACK"
          target="_blank"
        >
          <img src={furHood} />
          <p>Price: $69.97 </p>
          <p>French Connection Waist Belt Quilted Faux Fur Hooded Jacket</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2480966/guess-hi-lo-quilted-jacket?color=FOREST"
          target="_blank"
        >
          <img src={guessJacket} />
          <p>Price: $45 </p>
          <p>GUESS Hi-Lo Quilted Jacket</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2599482/bcb-generation-faux-fur-trim-parka-jacket?color=WHITE"
          target="_blank"
        >
          <img src={generationJacket} />
          <p>Price: $99.97 </p>
          <p>BCBGeneration Faux Fur Trim Parka Jacket</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2690595/matty-m-plaid-wrap-coat?color=BERRY"
          target="_blank"
        >
          <img src={plaidCoat} />
          <p>Price: $59.97 </p>
          <p>Matty M Plaid Wrap Coat</p>
        </a>
      </div>
      {/* shirts */}
      <div className="coats">
        <a
          href="https://www.nordstromrack.com/shop/product/1894782/lush-side-slit-tank?color=BARBERRY"
          target="_blank"
        >
          <img src={lush} />
          <p>Price: $9.97 </p>
          <p>Lush Side Slit Tank</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2591843/vince-camuto-textured-satin-blouse?color=RICH%20BLACK"
          target="_blank"
        >
          <img src={satinBlouse} />
          <p>Price: $14.99 </p>
          <p>Vince Camuto Textured Satin Blouse</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2598801/free-people-we-the-free-by-free-people-saturday-lace-trim-linen-blend-tee?color=IVORY"
          target="_blank"
        >
          <img src={laceTrim} />
          <p>Price: $9.99 </p>
          <p>
            Free People We the Free by Free People Saturday Lace Trim Linen
            Blend Tee
          </p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2582629/lucky-brand-usa-logo-graphic-tee?color=STEEL%20GREY"
          target="_blank"
        >
          <img src={luckyBrand} />
          <p>Price: 11.98 </p>
          <p>Lucky Brand USA Logo Graphic Tee</p>
        </a>
      </div>
      {/* pants */}
      <div className="coats">
        <a
          href="https://www.nordstromrack.com/shop/product/2561381/democracy-vintage-wash-cord-jeggings-petite?color=BLACK"
          target="_blank"
        >
          <img src={democracy} />
          <p>Price: $39.97 </p>
          <p>Democracy Vintage Wash Cord Jeggings (Petite)</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/1475866/vertigo-seamed-stretch-leggings?color=BURGUNDY"
          target="_blank"
        >
          <img src={vertigo} />
          <p>Price: 19.78 </p>
          <p>Vertigo Seamed Stretch Leggings</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2656050/halogen-halogen-r-slim-pants-regular-petite?color=BLACK-%20WHITE%20YVETTE%20PLD"
          target="_blank"
        >
          <img src={halogen} />
          <p>Price: $17.49 </p>
          <p>Halogen Halogen(R) Slim Pants (Regular & Petite)</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/978407/kut-from-the-kloth-cleaned-up-katy-boyfriend-jeans?color=ANGELAS%20DARK%20STONE"
          target="_blank"
        >
          <img src={kyi} />
          <p>Price: $39.97 </p>
          <p>KUT from the Kloth Cleaned Up Katy Boyfriend Jeans</p>
        </a>
      </div>
      {/* shoes */}
      <div className="coats">
        <a
          href="https://www.nordstromrack.com/shop/product/2212163/ugg-ugg-r-australia-bailey-bow-corduroy-boot-women?color=BLK"
          target="_blank"
        >
          <img src={uggs} />
          <p>Price: $139.97 </p>
          <p>UGG UGG(R) Australia 'Bailey Bow Corduroy' Boot (Women)</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2316342/ugg-ugg-r-constantine-boot-women?color=CHRC"
          target="_blank"
        >
          <img src={uggsGrey} />
          <p>Price: $109.97 </p>
          <p>UGG UGG(R) Constantine Boot (Women)</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2297746/toms-speckle-chambray-classics-slip-on-sneaker?color=BLACK"
          target="_blank"
        >
          <img src={toms} />
          <p>Price: $39.97 </p>
          <p>TOMS Speckle Chambray Classics Slip-On Sneaker</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2657705/halogen-halogen-r-piper-bootie-women?color=BLACK%20LEATHER%2FROSEGOLD"
          target="_blank"
        >
          <img src={halogen} />
          <p>Price: $79.97 </p>
          <p>Halogen Halogen(R) Piper Bootie (Women)</p>
        </a>
      </div>
      {/* accessories */}
      <div className="coats">
        <a
          href="https://www.nordstromrack.com/shop/product/2383929/ray-ban-52-mm-wayfarer-sunglasses?color=BLACK"
          target="_blank"
        >
          <img src={rayban} />
          <p>Price: $59.97 </p>
          <p>Ray-Ban 52mm Wayfarer Sunglasses</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2622998/ray-ban-160-mm-shield-sunglasses?color=GOLD%20BROWN"
          target="_blank"
        >
          <img src={raybanShield} />
          <p>Price: $79.97 </p>
          <p>Ray-Ban 160mm Shield Sunglasses</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2642164/cejon-faux-fur-collar-waffle-knit-shawl?color=GREY"
          target="_blank"
        >
          <img src={cejon} />
          <p>Price: $32.97 </p>
          <p>Cejon Faux Fur Collar Waffle Knit Shawl</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2620527/14th-union-striped-fringe-trim-infinity-scarf?color=GREY%20COMBO"
          target="_blank"
        >
          <img src={unionScarf} />
          <p>Price: $14.97 </p>
          <p>14th & Union Striped Fringe Trim Infinity Scarf</p>
        </a>
      </div>
      {/* accessories2 */}
      <div className="coats">
        <a
          href="https://www.nordstromrack.com/shop/product/2641559/sofia-cashmere-jersey-cashmere-gloves-with-tech-tips-genuine-fox-fur-trim?color=001BLK"
          target="_blank"
        >
          <img src={gloves} />
          <p>Price: $79.97 </p>
          <p>
            Sofia Cashmere Jersey Cashmere Gloves with Tech Tips & Genuine Fox
            Fur Trim
          </p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2181014/kyi-kyi-faux-fur-pompom-wool-blend-classic-beanie?color=CREAM"
          target="_blank"
        >
          <img src={kyi} />
          <p>Price: $24.97 </p>
          <p>Kyi Kyi Faux Fur Pompom Wool Blend Classic Beaniekyi</p>
        </a>

        <a
          href="https://www.nordstromrack.com/shop/product/2641894/michael-kors-pull-chain-belt-bag?color=BLACK"
          target="_blank"
        >
          <img src={korsPurse} />
          <p>Price: $29.97 </p>
          <p>Michael Kkorors Pull Chain Belt Bag</p>
        </a>
        <a
          href="https://www.nordstromrack.com/shop/product/2498936/rebecca-minkoff-mini-5-zip-leather-crossbody-bag?color=PUTTY"
          target="_blank"
        >
          <img src={rebeccaPurse} />
          <p>Price: $67.48 </p>
          <p>Rebecca Minkoff Mini 5 Zip Leather Crossbody Bag</p>
        </a>
      </div>
    </div>
  );
};

export default WomensClothing;